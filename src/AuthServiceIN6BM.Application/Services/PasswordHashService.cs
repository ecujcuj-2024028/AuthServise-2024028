...
public class PasswordHashService : IPasswordHashService
{
    ...
    public bool VerifyPassword(string password, string HashedPassword)
    {
        try
        {
            if (HashedPassword.StartsWith("$"))
            {
                ...
                var result = VerifyArgon2StandarFormat(password, HashedPassword);
        ...
        {
            Console.WriteLine($"[DEBUG] Exception la verifyPassword: {ex.Message}");
            return false;
        }
    }

    private bool VerifyArgon2StandarFormat(string password, string HashedPassword)
    {
        try
        {
            ...
            var expectedHash = Convert.FromHexString(FromBase64UrlSafe(hashBase64));
    ...
    {
        string base64 = base64UrlSafe.Replace('-','+').Replace('-','/');
